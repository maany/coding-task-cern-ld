...
def visible_area(mountains):
    ...
    region_to_peak_mapping = inflection_points_and_peak_tuples(indexed_mountains, pina_collider_filtered_peaks)
    total_area = 0
    for elem in region_to_peak_mapping:
        ...
        logging.debug(f"Area between points {left_bound}, {right_bound}: {mountain_area}")
